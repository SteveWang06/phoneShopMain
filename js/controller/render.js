...
function addToCart(id) {
  let item = phoneList.find((sp) => sp.id === id.toString());
  let checkItemReady = cartItem.find((spReady) => spReady.product.id === id.toString());
  if (!checkItemReady) {
    let cartShop = { product: item, quantity: 1 };
    cartItem.push(cartShop);
  } else {
    checkItemReady.quantity += 1;
  }
  ...
  cartItem.map((item) => {
    ...
    renderHTML += `
        <div class="item-cart">
            ...
            <i onclick="deletedCart('${
              item.product.id
            }')" class="fa fa-trash-alt remove-cart"></i>
